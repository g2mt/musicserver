...
void TrackDelegate::paint(QPainter *painter, const QStyleOptionViewItem &option,
	...
	QRect coverRect(
	    option.rect.left() + 4,
	    option.rect.top() + (option.rect.height() - coverSize.height()) / 2,
	...
	QRect nameRect(textX, coverRect.top(), textWidth,
	...
	QRect subRect(textX, nameRect.bottom(), textWidth,
	              option.rect.height() / 2);
	painter->drawText(subRect, Qt::AlignLeft | Qt::AlignTop,
	                  painter->fontMetrics().elidedText(
	                      subText, Qt::ElideRight, textWidth));
