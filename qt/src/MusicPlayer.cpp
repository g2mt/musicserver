...
void MusicPlayer::setupUi() {
	...
	connect(m_progressSlider, &QSlider::sliderReleased, this, [this]() {
		m_seeking = false;
		AppState::instance()->setProgress(
		    m_progressSlider->value() / 1000.0 *
...
void MusicPlayer::onPrevClicked() {
	AppState::instance()->queuePrev();
}

void MusicPlayer::onNextClicked() {
	AppState::instance()->queueNext();
}
...
void MusicPlayer::contextMenuEvent(QContextMenuEvent *event) {
	...
	connect(repeatTrack, &QAction::triggered, this, [state]() {
		state->setRepeat(RepeatMode::Track);
	});
	connect(repeatQueue, &QAction::triggered, this, [state]() {
		state->setRepeat(RepeatMode::Queue);
	});
	connect(repeatOff, &QAction::triggered, this, [state]() {
		state->setRepeat(RepeatMode::None);
	});
