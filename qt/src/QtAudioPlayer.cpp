...
void QtAudioPlayer::seekTo(qint64 ms) {
	m_player->setPosition(ms);
}

qint64 QtAudioPlayer::currentTime() const {
	return m_player->position();
}

qint64 QtAudioPlayer::duration() const {
...
void QtAudioPlayer::applyGain() {
	m_audioOutput->setVolume(computeGain());
}