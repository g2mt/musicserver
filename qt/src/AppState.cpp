...
AppState *AppState::instance() {
	if (!s_instance) s_instance = new AppState();
	return s_instance;
}
...
void AppState::setSearchQuery(const QString &query, int limit) {
	if (m_searchQuery == query && m_resultLimit == limit) return;
	m_searchQuery = query;
	m_resultLimit = limit;
...
void AppState::setResultSort(const QString &sort, bool desc) {
	if (m_resultSort == sort && m_resultDesc == desc) return;
	m_resultSort = sort;
	m_resultDesc = desc;
...
void AppState::queueRemove(int index) {
	if (index < 0 || index >= m_queueTracks.size()) return;
...
bool AppState::showOnlyQueueAfterEnqueue() const { return m_showOnlyQueueAfterEnqueue; }
bool AppState::shuffleBeforePlayingAll() const { return m_shuffleBeforePlayingAll; }
bool AppState::showTracksListOnTabChange() const { return m_showTracksListOnTabChange; }
int AppState::searchHistoryLimit() const { return m_searchHistoryLimit; }

void AppState::setLeftTab(LeftTab tab) {
	if (m_leftTab == tab) return;
	m_leftTab = tab;
...
void AppState::setShuffleBeforePlayingAll(bool shuffle) {
	if (m_shuffleBeforePlayingAll == shuffle) return;
...
void AppState::loadConfig() {
	...
	setMaxNormalizationDbs(
	    settings.value("maxNormalizationDbs", 8.0).toDouble());

	QString sq = settings.value("searchQuery").toString();
	int sql = settings.value("searchLimit", 0).toInt();
	if (!sq.isEmpty()) setSearchQuery(sq, sql);
	...
	settings.endArray();
	if (!bms.isEmpty()) setBookmarks(bms);
