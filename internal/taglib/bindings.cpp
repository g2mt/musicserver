...
static bool fill_cover_art(CoverArt *cover_art, const unsigned char *data,
...
BindingResult extract_cover_art(const char *filepath, CoverArt *cover_art) {
  ...
  // MP4 / AAC / M4A / M4B
  if (ext == "mp4" || ext == "m4a" || ext == "m4b" || ext == "m4r" ||
      ext == "aac") {
  ...
  // Fallback: try generic FileRef (won't yield cover art but at least validates
  // the file)
